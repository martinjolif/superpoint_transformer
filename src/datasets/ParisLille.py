...
from src.datasets.ParisLille_config import *
...
def read_paris_lille_tile(
    ...
    """Read a Paris_Lille tile saved as PLY.
    ...
    with open(filepath, "rb") as f:
        ...
        if xyz:
            #print(tile)
            #print(tile[key]["x"])
            #print(tile[key]["x"][0], tile[key]["x"][1])
            #print(type(tile[key]["x"]))
            #print(tile[key]["x"].shape)
            #print(torch.DoubleTensor(tile[key]["x"].copy()))
            #(torch.FloatTensor(tile[key]["x"]))
            pos = torch.stack([
        ...
        if reflectance:
            # Heuristic to bring the reflectance distribution in [0, 1]
            if 'reflectance' in tile[key]:
                data.reflectance = torch.FloatTensor(
                    tile[key]['reflectance']).clip(min=0, max=255) / 255
            else:
                data.reflectance = torch.FloatTensor(
                    tile[key]['scalar_reflectance']).clip(min=0, max=255) / 255
            

        if semantic:
            if 'class' in tile[key]:
                y = torch.LongTensor(tile[key]['class'].copy())
            else:
                y = torch.LongTensor(tile[key]['scalar_class'].copy())
...
class PARISLILLE(BaseDataset):
    ...
    def download_dataset(self) -> None:
        ...
        """
        """
        print(self.root, self._zip_name)
        print(osp.exists(osp.join(self.root, self._zip_name)))
    ...
    def read_single_raw_cloud(self, raw_cloud_path: str) -> 'Data':
        ...
        """
        print("##########################", raw_cloud_path)
        return read_paris_lille_tile(
            raw_cloud_path, reflectance=True, semantic=True, instance=False,
            remap=True)
